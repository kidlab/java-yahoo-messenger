...
{	// -----Misc Yahoo data
	...
	{	// -----The regular session should be logged in, and the chat status
		...
		{	// -----Transmit 'connect' packet and wait for acknowledgement
			...
			// -----Successful?
			if(chatSessionStatus!=MESSAGING) currentLobby=null;
		}
		finally
		...
		{	// -----Is this an error packet, sent to us via processError() ?
			...
			{	try
				{	String captchaMsg = pkt.getValue("105");
					...
					new FireEvent().fire(se,SERVICE_X_CHATCAPTCHA);
					return;  // ...to finally block
				}catch(Exception e) { throw new YMSG9BadFormatException("chat captcha",false,e); }
			}
		...
		{	for(int i=0;i<listeners.size();i++)
			{	SessionListener l = (SessionListener)listeners.elementAt(i);
				...
				{	case SERVICE_LOGOFF :		l.connectionClosed(ev);  break;
					...
					case SERVICE_X_CHATCAPTCHA: l.chatCaptchaReceived((SessionChatEvent)ev);  break;
